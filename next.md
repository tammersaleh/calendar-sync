...
000. **iCal feed importer (`[[feeds]]`)** - built on branch `feat/ical-importer`, **NOT merged or pushed** at end of session. Six layers (`internal/ical` parser, `internal/feed` fetcher, `internal/feedimport` importer + Runner, `[[feeds]]` config, daemon/cmd wiring, docs), each red-green with a before+after code-review pass; `mise run check` green. Full design + remaining steps in `doc/plans/ical-importer.md`.

    **Why it exists:** the travel events on the calendars come from Navan's OAuth Google-Calendar integration, which is DISCONNECTED (16 stale `navan*` events on CoreWeave, frozen at a pre-rebooking itinerary, mirrored to Personal). Navan's OAuth calendar write appears org-disabled (only the read-only feed link shows in settings). The fresh source is the TripIt (or Navan) iCal feed. The importer polls the feed and writes into a Google calendar fast, replacing the dead integration; the pair mesh mirrors onward.

    **Remaining (all await user go-ahead - outward-facing/secret, so left undone deliberately):** (1) final full-diff review, merge to main + push → auto-release; (2) install release + restart daemon; (3) user adds a `[[feeds]]` entry (recommend `url_env = "TRIPIT_ICAL_URL"`, target `me@tammersaleh.com`); (4) verify the Jul-13 trip imports with the rebooked STS→LAX→EWR flights, then do the DEFERRED cleanup of the 16 stale `navan*` events on `tsaleh@coreweave.com` (delete on the work calendar; daemon auto-prunes the personal mirrors, same as the Reclaim cleanup below).
