...
### `force_all_day_busy` forces all-day feed events opaque, via the checksum

The per-feed `[[feeds]].force_all_day_busy` bool (config.Feed → CanonicalFeed → feedimport.FeedConfig → Importer) makes `Importer.transparencyFor` return opaque for any all-day item, overriding its `TRANSP`. Timed items are untouched. The discriminator is `it.Start.AllDay` ONLY - DTEND is exclusive and may be absent, so `End` is never consulted.

There is deliberately no separate write path: the forced transparency flows into `ev.Transparency` → `managedFields` → `mirror.Checksum`, so the flag participates in change detection like any managed field. Flipping the flag on changes the checksum for previously-transparent all-day events, so the next reconcile repatches them to Busy; already-opaque all-day events see no checksum change and are left alone (no churn). No `[settings]` default exists - the field is a plain bool, absent == false == import transparency verbatim. Flipping it edits config.toml, which triggers the launchd `WatchPaths` restart (fresh Fetcher, empty cache gate → full GET + reconcile), so the change takes effect without waiting on feed content to move.
...
### A recurring parent's anchor is never reverse-propagated (B38)

`doPropagate` (`internal/sync/drift.go`) is the ONLY source-side `events.patch` in the sync layer, reached from FullSync, the tick source-delta, and the target-delta phase (`processTargetDeltaEvent` routes a mirror-parent edit through the same `Classify`). When the source is a recurring PARENT (`len(source.Recurrence) > 0`), the drifted fields are partitioned: anchor `{start, end, recurrence}` are NEVER sent to the source, safe `{summary, description, location, transparency, visibility}` still propagate. An `events.patch` of `start`/`end` on a recurring parent's endpoint moves the DTSTART grid and shifts every occurrence; a `recurrence` patch rewrites the rule. A single false-positive mirror-parent drift there destroyed a live user's whole weekly series (B38), so the source is authoritative for a recurring parent's timing regardless of `source_writable`.

This is the same structural-impossibility approach as B16 (`BuildSourceOverridePatchBody` never emits recurrence). The guard uses an explicit safe allowlist (`recurringParentSafeFields`), so anchors AND any future managed field fail closed. It fires when EITHER `source.Recurrence` or `mirrorEvent.Recurrence` is non-empty - a source that dropped its RRULE while the mirror still carries the old one must not have that stale RRULE re-propagated back (it would resurrect the series). The guard lives in `doPropagate` rather than `reconcileNormal` or `mirror.Classify` because that is the single narrow choke point where the source write happens, and it naturally covers both the plain-propagate and conflict-target-won cells. Anchor-only drift delegates to `doRevert` (`outcome.Action` forced to revert) so the conflict label and timestamps survive; a mixed drift propagates the safe fields and the follow-up mirror rewrite snaps the anchor back. `doRevert` itself only patches the mirror, so read-only sources were already safe.

The recurring-INSTANCE handler is NOT modified by this fix. Its ordinary propagate path patches the occurrence's own materialized ID (never the parent), and in practice recurrence is never in an instance's drifted set because neither the daemon (`BuildInstancePayload` sets `Recurrence=nil`) nor Google ever puts an RRULE on an instance. That is empirical, not structurally enforced - the belt-and-suspenders hardening (strip recurrence from every source-instance patch, matching `BuildSourceOverridePatchBody`) is tracked as B39.
