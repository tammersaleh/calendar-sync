...
func TestBuildPropagateBody_StripsTrailerFromDescription(t *testing.T) {
	live := &gws.Event{
	...
	body := buildPropagateBody(live, []string{"summary", "description", "start", "end"})
...
func TestBuildPropagateBody_OmitsUndriftedFields(t *testing.T) {
	live := &gws.Event{
		Summary:      "Real summary",
		Description:  "body\n\n---\nSource: https://www.google.com/calendar/event?eid=ABC",
		Start:        &gws.EventDateTime{DateTime: "2026-05-01T12:00:00Z"},
	...
	body := buildPropagateBody(live, []string{"summary"})
	if body.Summary != "Real summary" {
		t.Errorf("Summary = %q", body.Summary)
	}
	if body.Description != "" {
