...
8. **Commit syncTokens conditionally.** For each unique source `S`, install the staged `nextSyncToken` from step 5 into the in-memory per-source token *only if every pdir whose source matches `S` succeeded in step 7*. If any pdir for `S` failed, leave the in-memory token empty so the next cycle re-runs a full source-list for `S`. This is the same conditional-advancement rule that protects the per-tick path; both paths apply it.
   ...
   If the staged token is missing the same rule applies: leave the in-memory token empty so the next cycle re-runs a full source-list. Note this is not an expected outcome for a long list. Google returns `nextSyncToken` on the terminal page; a missing token means the list was truncated before the collection was exhausted, which `internal/gws` now rejects as `ErrIncompletePagination` before the sync layer ever sees it (B28).
...
**Feed-import phase (feeds only).** Before the reconciliation phases below, the daemon runs the feed importer once for every enabled `[[feeds]]` entry: fetch (subject to the per-feed cache gate), parse, and reconcile into the target calendar. This runs FIRST so that a feed change lands on its target calendar before the source-delta phase lists that calendar - a feed event imported to calendar `T` is then picked up and mirrored onward by any pair whose source is `T` within the same tick, rather than waiting for the next one. A feed failure is isolated per feed and never aborts the reconciliation phases.

The feed-import phase runs on the per-tick path ONLY, never on the startup or periodic full re-sync. A full re-sync resets each source's `syncToken` from a full `events.list`, and that list can omit an event the importer wrote seconds earlier while the returned token already sits past it - stranding the event until the next full re-sync (B26). The per-tick incremental delta's token is consistent with its own results, so a briefly-lagged feed write is caught by the next delta instead. At startup the feed import therefore lands on the first tick (~one `poll_interval` after launch) rather than during the startup full re-sync. (One narrow residual remains - a fast-track full re-sync triggered by a 410 in the same tick as a feed import can still race the write; see B27 in `doc/bugs.md`.)
